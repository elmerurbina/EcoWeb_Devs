...
# Ruta para la interfaz principal del foro
@app.route('/foro')
def foro():
    ...
    # No se requiere item_id y item_type para esta ruta
...
def respuestas():
    ...
    # Verificar que se proporcionen item_id y item_type solo aquí
...
def new_debate():
    ...
    autor_id = current_user.id  # Obtener el ID del usuario actual

    try:
        # Guardar el debate con el autor
...
def edit_debate(debate_id):
    ...
    # Verificar si el usuario actual es el autor del debate
...
@login_required
def delete_debate_route(debate_id):
    debate = get_debate_by_id(debate_id)
    if debate is None:
        flash('Debate no encontrado', 'error')
        return redirect(url_for('foro'))
